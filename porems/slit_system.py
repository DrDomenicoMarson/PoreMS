...
class SilicaSlit:
    ...
    def __init__(
        ...
        ------
        TypeError
            Raised when ``surface`` is not a prepared internal pore engine.
        """
        if not isinstance(surface, Pore):
            raise TypeError("surface must be a prepared Pore instance.")
    ...
    def _from_block(cls, block, connectivity, slit_width_nm, name):
        ...
        surface = Pore(block, connectivity)
    ...
    def site_normal(self, site_id, position_nm):
        """Return the pore-facing normal for one binding site.
